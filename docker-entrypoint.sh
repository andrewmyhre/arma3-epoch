...
cd $ARMASVRPATH
if [ -f "$FILE" ]; then
	echo "./$FILE -port=2302 -profiles='$ARMASVRPATH/sc' -mod='$ARMAMODS' -serverMod='$ARMASERVERMODS' -config='$ARMASVRPATH/sc/server.cfg' -cfg='$ARMASVRPATH/sc/basic.cfg' -name=default -world=empty -autoinit"
#   ./$FILE -port=2302 -profiles=/profiles -mod="$ARMAMODS" -serverMod="$ARMASERVERMODS" -config="/arma3/sc/main.cfg" -cfg="/arma3/sc/basic.cfg" -name=default -world=empty -autoinit
   ./$FILE -port=2302 -profiles="$ARMASVRPATH/sc/users" -mod="$ARMAMODS" -serverMod="$ARMASERVERMODS" -config="$ARMASVRPATH/sc/server.cfg" -cfg="$ARMASVRPATH/sc/basic.cfg" -name=default -world=empty -autoinit
else
